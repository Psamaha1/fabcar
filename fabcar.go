...
func (s *SmartContract) createDefectFF(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	var data = DataFF{SensorName: args[1], FaultType: args[2], Valueff: args[3], Importance: args[4], Timestampff: args[5]}
	dataAsBytes, _ := json.Marshal(data)
	APIstub.PutState(args[0], dataAsBytes)
	indexName := "faulttype~key"
	colorNameIndexKey, err := APIstub.CreateCompositeKey(indexName, []string{data.FaultType, args[0]})
	if err != nil {
		return shim.Error(err.Error())
	}
	value := []byte{0x00}
